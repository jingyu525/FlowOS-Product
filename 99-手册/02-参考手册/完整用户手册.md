...
graph TD
    Inbox[00-收集箱<br>收集箱]
    Projects[10-项目<br>项目]
    Areas[20-领域<br>领域]
    Resources[30-资源<br>资源]
    Archives[40-归档<br>归档]
...
*   **90-模版 (模版)**
    *   **用途**: 存放笔记和项目的标准化模版。
    *   **示例**: [日记模版](../90-模版/日记模版.md)、[项目模版](../90-模版/项目模版.md)。
*   **99-手册 (手册)**
...
*   **Dashboard 原理**:
    ...
    *   **代码解释**:
        ...
        *   `FROM "10-项目"`: 数据来源：10-项目 文件夹
        *   `WHERE status != "completed"`: 过滤条件：状态不等于 "completed"
        *   `AND file.name != "10-项目"`: 排除文件夹索引文件本身
    *   这意味只要你在 `10-项目` 文件夹下的笔记中定义了 `status` 属性（YAML frontmatter），它就会自动出现在仪表盘中。
...
1.  **启动 (Initiation)**:
    *   在 `00-收集箱` 中捕捉想法。
    *   确认转化为项目后，在 `10-项目` 中创建新笔记（可使用 [项目模版](../90-模版/项目模版.md)）。
    *   在笔记的 YAML 区域设置 `status: active`。
    *   将项目添加到 [活跃项目看板.md](../10-项目/活跃项目看板.md) 的 "Todo" 列。
2.  **执行 (Execution)**:
...
*   **每周回顾 (Weekly Review)**:
    *   检查 `10-项目` 中的所有活跃项目，确保进度正常。
    *   检查 `20-领域`，看是否有需要转为项目的事务。
    *   更新 [活跃项目看板.md](../10-项目/活跃项目看板.md)。
    *   清理电脑桌面和下载文件夹，归档到 `00-收集箱` 或删除。
...
*   当 `10-项目` 中的项目完成，或 `20-领域` 中的领域不再关注时，请将其移动到 `40-归档`。
*   **建议**: 按年份在 `40-归档` 下建立子文件夹（如 `40-归档/2024`），以便于未来查找。
